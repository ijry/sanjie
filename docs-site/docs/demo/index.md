...
<div class="demo-layout">
  ...
  <div class="demo-panel">
    ...
    <h2>三界移动办公演示</h2>
    <p>内置完整 mock 数据，覆盖工作台、勾魂任务、投胎队列、审批中心、生死簿、十八层地狱、愿望工单、孟婆汤、审计日志和多角色切换。列表页保持高密度卡片，核心详情页按业务拆成夜巡、票据、案牍、命簿、控制台和签文等差异化视觉，适合在文档站内嵌窗口直接演示。</p>
    <p>
...
- 勾魂管理：待执行、执行中、异常、已完成、已撤销任务均有多条样例，详情页是夜巡定位任务台。
- 投胎管理：排队、复核、已通过、待喝汤、已完成、已驳回流程均有多条样例，详情页是奈何桥排队票据。
- 生死簿：正常、预警、严重风险和冻结记录均有样例，详情页是命簿卷宗，展示阳寿偏差、处置流程和审计记录。
- 十八层地狱：1 至 18 层容量、负载、设备状态和刑期记录完整展示，刑期详情是地狱控制台并支持发起复核。
- 愿望工单：求财、求学、姻缘、职场、健康、事业、运气等类型齐全，详情页是许愿签文并支持转派、办结和驳回。
- 其他模块：孟婆汤库存与发放记录、审批记录、审计日志、预警消息。

mock 状态保存在前端内存中，页面刷新后恢复初始数据。按钮操作会即时修改当前会话数据，并补充审计记录或审批记录，适合演示流程和截图。Go + SQLite 真实后端的种子数据也保持同等密度，方便从 mock 演示切到真实 API 联调。

## 推荐体验路径

1. 从工作台进入勾魂任务，打开待执行任务详情，点击开始或上报异常。
2. 进入审批中心，处理寿命变更、投胎插队或地狱复核审批。
3. 打开生死簿详情，冻结一条高危记录或发起改寿申请。
4. 打开十八层地狱，查看高负载楼层并进入刑期详情发起复核。
5. 打开愿望工单详情，把愿望转派给对应神仙，再办结或驳回。
6. 最后进入审计日志，确认敏感操作已经留痕。
