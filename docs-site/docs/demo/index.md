...
- 工作台：三界总控大屏、流转图、天条预警、首次演示导览和推荐演示路线都可跳转到对应模块。
...
- 其他模块：汤坊库存支持补货/扣减，审计日志按目标类型过滤，多角色身份页展示岗位权限、推荐入口、mock 会话状态和重置演示入口。

mock 状态保存在浏览器本地存储中，刷新页面不会丢失当前会话操作。按钮操作会即时修改当前会话数据，并补充审计记录或审批记录，适合连续演示流程和截图。需要恢复初始状态时，进入“我的”页点击“重置演示”，系统会恢复 mock 种子数据并切回玉帝身份。Go + SQLite 真实后端的种子数据也保持同等密度，方便从 mock 演示切到真实 API 联调。

## 推荐体验路径

1. 从三界总控工作台查看流转图和天条预警，进入勾魂任务。
2. 按首次演示导览切到黑无常、判官或孟婆身份。
3. 进入审批中心，处理寿命变更、投胎插队或地狱复核审批。
4. 打开生死簿详情，冻结一条高危记录或发起改寿申请。
5. 打开十八层地狱，查看高负载楼层并进入刑期详情发起复核。
6. 打开愿望工单详情，把愿望转派给对应神仙，再办结或驳回。
7. 切换到孟婆身份进入汤坊库存台，补一批低库存汤剂。
8. 最后进入天眼审计日志，按类型过滤并确认敏感操作已经留痕。
9. 回到“我的”页查看 mock 会话指标，必要时点击“重置演示”恢复初始状态。
