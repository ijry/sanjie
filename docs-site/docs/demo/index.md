...
<div class="demo-layout">
  ...
  <div class="demo-panel">
    ...
    <h2>三界移动办公演示</h2>
    <p>内置完整 mock 数据，覆盖工作台、勾魂任务、投胎队列、审批中心、生死簿、十八层地狱、愿望工单、孟婆汤、审计日志和多角色切换。列表页已做移动端高密度卡片展示，适合在文档站内嵌窗口直接演示。</p>
    <p>
...
- 生死簿：正常、预警、严重风险和冻结记录均有样例，卡片内展示阳寿、劫数、死因和更新时间。
- 十八层地狱：1 至 18 层容量、负载、设备状态和刑期记录完整展示。
- 其他模块：愿望工单、孟婆汤库存与发放记录、审批记录、审计日志、预警消息。

mock 状态保存在前端内存中，页面刷新后恢复初始数据。按钮操作会即时修改当前会话数据，适合演示流程和截图。Go + SQLite 真实后端的种子数据也保持同等密度，方便从 mock 演示切到真实 API 联调。
