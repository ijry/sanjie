...
<div class="demo-layout">
  ...
  <div class="demo-panel">
    ...
    <h2>三界移动办公演示</h2>
    <p>内置完整 mock 数据，覆盖工作台、勾魂任务、投胎队列、审批中心、生死簿、十八层地狱、愿望工单、孟婆汤、审计日志和多角色切换。</p>
    <p>
...
- 勾魂管理：待执行、执行中、异常、已完成、已撤销任务均有样例。
- 投胎管理：排队、复核、已通过、待喝汤、已完成、已驳回流程均可查看。
- 生死簿：正常、预警、严重风险和冻结记录均有样例。
- 十八层地狱：1 至 18 层容量、负载、设备状态和刑期记录完整展示。
- 其他模块：愿望工单、孟婆汤库存与发放记录、审批记录、审计日志、预警消息。

mock 状态保存在前端内存中，页面刷新后恢复初始数据。按钮操作会即时修改当前会话数据，适合演示流程和截图。
